...
public class MenuOptions : MonoBehaviour
{
    ...
    void KeyboardSteer()
    {
        ...
        if (index == 0)
        {
            ...
            spinSpeed = Mathf.Clamp(spinSpeed, 0.02f, 1);
        ...
        if (index == 1)
        {
            if(onetime){
                ...
                info.SetActive(true);
                onetime = false;}
            legsDiff = Mathf.Clamp(legsDiff, 2, 100);
        ...
        if (index == 2)
        {
            ...
            PersistentManagerScript.Instance.config.SaveToFile(Application.persistentDataPath + "/config.cfg");
